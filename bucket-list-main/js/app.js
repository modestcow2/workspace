...
class BucketListApp {
    ...
    updateStats() {
        ...
        if (progressText) {
            if (stats.total === 0) {
                progressText.textContent = '목표를 추가하고 하나씩 달성해보세요!';
            } else if (stats.completionRate === 100) {
                progressText.textContent = '축하합니다! 모든 목표를 달성했어요! 🎊';
            } else if (stats.completionRate >= 50) {
                progressText.textContent = `절반 이상 달성! ${stats.progress}개만 더 힘내세요! 💪`;
            } else {
                progressText.textContent = `${stats.completed}/${stats.total} 완료 — 한 걸음씩 나아가고 있어요!`;
            }
        }
    }
     ...
     * 버킷 리스트 항목 HTML 생성
     */
    createBucketItemHTML(item, index) {
        ...
        const accentClass = item.completed ? 'accent-completed' : 'accent-active';
        const delay = Math.min(index * 0.05, 0.3);

        return `
            <div class="${itemClass} bg-white rounded-lg shadow-md p-4 flex items-center gap-3 hover:shadow-lg transition-shadow ${accentClass}" style="animation-delay: ${delay}s">
                <!-- 체크박스 -->
                <button
    ...
    render() {
        ...
        // 리스트 렌더링
        const html = bucketList.map((item, index) => this.createBucketItemHTML(item, index)).join('');
